...
class TData(Data):
    ...
    def __init__(self, eis, xs, ys, masks, ews=None, nmap=None, **kwargs):
        ...
        self.eis = eis  # List of edge index tensors for each timestep (adjacency info)
        self.T = len(eis)  # Number of timesteps in the data
        self.xs = xs  # Node features; tensor (static) or list of tensors (dynamic)
        self.masks = masks  # List of boolean masks for splitting edges (e.g., train/val)
        self.dynamic_feats = isinstance(xs, list)  # True if node features are dynamic over time
        self.ews = ews  # Edge weights per timestep, or None if not used
        self.ys = ys  # Labels, e.g., for anomaly detection, or None if test data
        self.is_test = not isinstance(ys, None.__class__)  # True if this is test data
        self.nmap = nmap  # Node mapping (e.g., from node IDs to indices)
...
def fast_negative_sampling(edge_list, batch_size, num_nodes, oversample=1.25):    
    ...
    # For faster membership checking (numpy path requires CPU)
