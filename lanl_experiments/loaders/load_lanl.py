...
# Input where LANL data cleaned with split.py is stored (must end with /)
LANL_FOLDER = '/home/mmanjee/CARS/processed/lanl/'
...
def load_lanl_dist(workers, start=0, end=635015, delta=8640, is_test=False, ew_fn=std_edge_w):
    ...
    num_slices = ((end - start) // delta) #// for integer division
    ...
    kwargs = [] #list of dictionaries for each worker to load a partial LANL dataset   
    prev = start
    for i in range(workers):
        end_t = prev + delta*per_worker[i] #end timestamp for the current worker
        kwargs.append({
            'start': prev, 
            'end': min(end_t-1, end),
            'delta': delta,
    ...
    if is_test:
        ...
        ys = None
        cnt = None #not used in test or train
...
'''
def load_partial_lanl(start, end, delta, is_test=False, ew_fn=std_edge_w):
    cur_slice = int(start - (start % FILE_DELTA))
    start_f = str(cur_slice) + '.txt'
    in_f = open(LANL_FOLDER + start_f, 'r')

    edges = [] #list of edge index pairs
    ews = [] #list of edge weights
    edges_t = {} #dictionary of edge index pairs and their anomaly labels and counts
    ys = [] #list of anomaly labels
    ...
    while keep_reading:
        while line:
            ...
            if ts >= next_split:
                if len(edges_t):
                    ei = list(zip(*edges_t.keys())) #zip the edge index pairs to the ei list
                    edges.append(ei) #append the edge index pairs to the edges list

                    y,ew = list(zip(*edges_t.values())) #zip the anomaly labels and edge weights
                    ews.append(torch.tensor(ew)) #append the edge weights to the ews list

                    if is_test:
                        ys.append(torch.tensor(y)) #append the anomaly labels to the ys list

                    edges_t = {} #clear the edges_t dictionary
...
# spinup assigns time 0 → 48600 to GPU0
#        ↓
# GCN.__init__ calls load_lanl_dist(jobs=2, start=0, end=48600, ...)
#        ↓
# joblib splits 0–48600 into 2 read jobs → 2 partial TData → merged (lines 79–88)
#        ↓
# ONE TData stored as GCN.data on GPU0
#        ↓
# GCN trains on that data