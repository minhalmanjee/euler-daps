...
RED = '/home/mmanjee/CARS/datasets/lanl/redteam.txt' # Location of redteam.txt
SRC = '/home/mmanjee/CARS/datasets/lanl/auth.txt' # Location of auth.txt
DST = '/home/mmanjee/CARS/processed/lanl/' # Directory to save output files to
...
def mark_anoms():
    ...
    with open(RED, 'r') as f:
        red_events = f.read().split() #splitting the file into list of lines
    ...
    for event in red_events:
        tokens = event.split(',') #splitting the line into tokens
        ts = int(tokens.pop(0))
        # print(ts)
        # print('\n', tokens)
...
def split():
    ...
    # adds the nmap id for src and dst to the file
