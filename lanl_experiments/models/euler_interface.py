...
from devices import move_tdata_to
...
class Euler_Encoder(DDP):
    ...
    def load_new_data(self, loader, kwargs):
        ...
        move_tdata_to(self.module.data, dev)
        return True
    ...
    def _align_z(self, z):
        ...
        return z if z.device == dev else z.to(dev)
...
class Euler_Recurrent(nn.Module):
    ...
    and calculates loss
    '''
    def __init__(self, rnn: nn.Module, remote_rrefs: list, device=None):
        '''
        ...
        device : torch.device
            Device for the temporal module (master GPU)
    ...
    def forward(self, mask_enum, include_h=False, h0=None, no_grad=False):
        ...
        if getattr(self.rnn, 'collect_before_rnn', False):
            # Transformer path: collect all worker embeddings first so the
            # transformer sees the full sequence in one call — proper gradient
            # flow and cross-sequence attention with no detach fragmentation
            raw = [f.wait().to(self.device) for f in futs]
            self.len_from_each = [e.size(0) for e in raw]
            zs, h0 = self.rnn(torch.cat(raw, dim=0), h0, include_h=True)
        else:
            # RNN path: stream embeddings through as each worker finishes,
            # threading hidden state h0 across chunks
            zs_list = []
            for f in futs:
                z_in = f.wait().to(self.device)
                z, h0 = self.rnn(z_in, h0, include_h=True)
                zs_list.append(z)
            self.len_from_each = [embed.size(0) for embed in zs_list]
