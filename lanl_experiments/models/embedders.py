...
from devices import ddp_device_ids, move_tdata_to, use_cuda, worker_device
...
class DropEdge(nn.Module):
    ...
    def forward(self, ei, ew=None):
        if self.training and self.p > 0:
            mask = torch.rand(ei.size(1), device=ei.device)
            if ew is None:
...
class GCN(Euler_Embed_Unit):
    ...
    def __init__(self, data_load, data_kws, h_dim, z_dim, device=None):
        '''
        ...
        device : torch.device
            CUDA device for this worker (None -> infer from dist rank or CPU)
        ...
        if device is None:
            ...
            device = worker_device(rank)
        self.device = device
        if use_cuda():
            torch.cuda.set_device(device)

        # Load in the data before initing params
        print("%s loading %s-%s on %s" % (
        ...
        move_tdata_to(self.data, device)

        # Params 
        self.c1 = GCNConv(self.data.x_dim, h_dim, add_self_loops=True).to(device)
        self.relu = nn.ReLU()
        self.c2 = GCNConv(h_dim, z_dim, add_self_loops=True).to(device)
        self.drop = nn.Dropout(0.25)
        self.tanh = nn.Tanh()
        self.de = DropEdge(0.8)
        self.edge_dev_alpha = 0.0

    def decode_edges(self, e, z, out_dev=None, in_dev=None):
        logits = (z[e[0]] * z[e[1]]).sum(dim=1)
        if self.edge_dev_alpha > 0 and out_dev is not None:
            pen = out_dev[e[0].long()]
            if in_dev is not None:
                pen = pen + in_dev[e[1].long()]
            logits = logits - self.edge_dev_alpha * pen
...
def _make_encoder(encoder_cls, module):
    rank = _worker_rank()
    return encoder_cls(module, **ddp_device_ids(rank))
...
def detector_gcn_rref(loader, kwargs, h_dim, z_dim, **kws):
    ...
    '''
    rank = _worker_rank() 
    gcn = GCN(loader, kwargs, h_dim, z_dim, device=worker_device(rank))
    return _make_encoder(DetectorEncoder, gcn)
...
def predictor_gcn_rref(loader, kwargs, h_dim, z_dim, head=False):
    ...
    rank = _worker_rank()
    gcn = GCN(loader, kwargs, h_dim, z_dim, device=worker_device(rank))
    return PredictorEncoder(gcn, head, **ddp_device_ids(rank))
...
class GAT(GCN):
    ...
    def __init__(self, data_load, data_kws, h_dim, z_dim, heads=3, device=None):
        super().__init__(data_load, data_kws, h_dim, z_dim, device=device)

        # Concat=False seems to work best
        self.c1 = GATConv(self.data.x_dim, h_dim, heads=heads, concat=False).to(self.device)
        self.c2 = GATConv(h_dim, z_dim, heads=heads, concat=False).to(self.device)
...
def detector_gat_rref(loader, kwargs, h_dim, z_dim, **kws):
    rank = _worker_rank()
    return _make_encoder(DetectorEncoder, GAT(loader, kwargs, h_dim, z_dim, device=worker_device(rank)))

def predictor_gat_rref(loader, kwargs, h_dim, z_dim, head=False):
    rank = _worker_rank()
    return PredictorEncoder(
        GAT(loader, kwargs, h_dim, z_dim, device=worker_device(rank)), head,
        **ddp_device_ids(rank),
    )
...
class SAGE(GAT):
    ...
    def __init__(self, data_load, data_kws, h_dim, z_dim, device=None):
        ...
        self.c1 = PoolSAGEConv(self.data.x_dim, h_dim).to(self.device)
        self.c2 = PoolSAGEConv(h_dim, z_dim).to(self.device)


def detector_sage_rref(loader, kwargs, h_dim, z_dim, **kws):
    rank = _worker_rank()
    return _make_encoder(DetectorEncoder, SAGE(loader, kwargs, h_dim, z_dim, device=worker_device(rank)))

def predictor_sage_rref(loader, kwargs, h_dim, z_dim, head=False):
    rank = _worker_rank()
    return PredictorEncoder(
        SAGE(loader, kwargs, h_dim, z_dim, device=worker_device(rank)), head,
        **ddp_device_ids(rank),
    )